...
def correct_sql_field(sql, question, model):
    # 复制原始SQL用于修改
    new_sql = sql
    key_words = list(company_table.load_company_table().columns)  # 加载数据库字段名列表

    fields, sql_numbers = get_field_number(sql)   # 提取SQL中的字段和数值
    for field in fields:
        if field not in key_words:
            most_like_word = get_most_like_word(field, key_words,model)  # 计算最相似合法字段
            if len(most_like_word) > 0:  # 找到有效候选
                logger.info('文本字段纠正前sql：{}'.format(new_sql)) # 记录原始SQL
                new_sql = new_sql.replace(field, most_like_word)
                logger.info('文本字段纠正后sql：{}'.format(new_sql))  # 记录修正后SQL
    return new_sql  # 返回修正后的SQL

'''
用于纠正 SQL 查询中的数字，确保 SQL 查询中的数字与问题中的数字一致
'''
def correct_sql_number(sql, question):
    # 用于纠正 SQL 查询中的数字，确保 SQL 查询中的数字与问题中的数字一致
    new_sql = sql
    # 调用 get_field_number 函数，从 SQL 查询中提取字段和数字
