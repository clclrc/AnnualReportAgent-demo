...
这是一个整理成公开 demo 形式的金融年报智能分析 Agent 仓库。公开版保留了多路由工作流、API 入口、benchmark 摘要、可回放样例，以及一小份可公开的数据子集；不再附带私有全量 PDF 语料、训练检查点和大批实验归档文件。
...
原始私有项目基于更大的 2019-2021 年 A 股年报语料。公开 demo 只保留一个 14 条记录的结构化子集、一个小型报告切片、4 个公开样例卡片和汇总指标，同时避免把整套实验归档全部公开。
...
- `legacy/`
  保留原项目里较旧的预处理和实验脚本，作为补充参考，但默认 quickstart 不依赖它们。
